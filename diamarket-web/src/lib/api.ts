import { Category, Order, Product, ShippingOption, Slide } from '@/lib/types';
...
const mockProducts: Product[] = Array.from({ length: 14 }).map((_, i) => ({
  ...
  stock: i % 3 ? 12 : 0,
  vendor: { id: 'v1', name: 'Vendor Elite', country: 'Cameroun', city: 'Douala' },
  weightKg: 1.1 + i / 10,
...
export const api = {
  ...
  async createOrder(payload: unknown): Promise<{ id: string }> { try { return await request('/orders', { method: 'POST', body: JSON.stringify(payload) }); } catch { return { id: `ord_${Date.now()}` }; } },
  async getOrders(): Promise<Order[]> { return [{ id: 'ord_1', items: [], totalFcfa: 80000, status: 'shipped', trackingNumber: 'DMK-TRK-2044' }]; },
