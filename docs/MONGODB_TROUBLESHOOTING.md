...
`diamarket-api` reads MongoDB configuration from `MONGODB_URI` in `apps/diamarket-api/.env` or from the process environment. Do not hardcode Atlas URIs in source code and never commit real credentials.
...
For Atlas, replace this value with the official connection string copied from the Atlas UI.
...
5. **DNS SRV lookup**: test whether your machine can resolve the Atlas SRV record:
...
`querySrv ENOTFOUND _mongodb._tcp.cluster0.wmizrba.mongodb.net` means the DNS SRV record cannot be resolved from the machine starting the API. The API logs only the MongoDB hostname, never the password, to help identify which cluster is being used.
...
- or change your Windows DNS servers to reliable public resolvers such as:
  - `1.1.1.1`
  - `8.8.8.8`
...
- `diamarket-api` starts and logs a clear warning;
- `/api/health` continues to return a successful response;
- routes that require MongoDB return `503 Database unavailable`.

Keep `ALLOW_API_WITHOUT_DB=false` for normal development, staging, and production so database issues fail fast.
