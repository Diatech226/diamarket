...
import { ProductExportButton, ProductStatsCards } from "@/components/products";
...
function validateProduct(form: ProductPayload) {
  ...
  if (!Number.isFinite(Number(form.price)) || Number(form.price) <= 0) errors.price = "Le prix doit être strictement supérieur à 0.";
...
export default function ProductsPage() {
  ...
  useEffect(() => {
    void Promise.all([cmsService.getAllCategories(), cmsService.getVendors(), cmsService.getMedia({ limit: 100, category: "product" })])
      .then(([categoryRows, vendorRows, mediaRows]) => { setCategories(categoryRows.filter((category) => category.active !== false)); setVendors(vendorRows.data); setMedia(mediaRows.data); })
