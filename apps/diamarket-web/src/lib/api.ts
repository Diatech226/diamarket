...
type ApiRequestInit = RequestInit & { public?: boolean };
async function request<T>(path: string, init?: ApiRequestInit): Promise<T> { const token = init?.public ? null : readStoredToken(); const { public: _public, ...fetchInit } = init ?? {}; const res = await fetch(`${API_BASE_URL}${path}`, { ...fetchInit, headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}), ...(fetchInit.headers ?? {}) }, credentials: 'include', cache: 'no-store' }); const body = await res.json().catch(() => null); if (!res.ok) { throw new Error((body && typeof body === 'object' && 'message' in body ? String((body as any).message) : '') || `API error ${res.status}`); } if (body === null) throw new Error('Réponse API invalide'); return body as T; }
...
export const api = {
  ...
  submitVendorRequest: (payload: unknown) => request<{ success: boolean; message: string; data: { id: string } }>('/vendor-requests', { method: 'POST', body: JSON.stringify(payload), public: true }),
};
