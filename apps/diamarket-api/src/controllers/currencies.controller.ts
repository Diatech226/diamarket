...
export const currenciesController = {
  ...
  async create(req: Request, res: Response) { const err = validate(req.body); if (err) return res.status(400).json({ success: false, message: err }); const payload = { ...req.body, code: normalize(req.body.code), rateToDefault: Number(req.body.rateToDefault), lastUpdatedAt: new Date() }; const data = await CurrencyRate.create(payload); if (data.isDefault) await setSingleDefault(data.id); await logAdminAction(getAuth(req)!.userId, 'currency.created', 'currency', data.id, payload); return res.status(201).json({ success: true, data }); },
