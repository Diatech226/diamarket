...
export default function AccountNotifications(){
  ...
  async function load(){ setLoading(true); const token=typeof window!=='undefined'?localStorage.getItem('diaexpress_token')||'':''; const data=await myNotifications(token, unread); setItems(data.items||data.notifications||[]); setLoading(false); }
  React.useEffect(()=>{load();},[unread]);
