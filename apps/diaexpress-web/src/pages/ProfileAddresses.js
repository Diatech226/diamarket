...
const ProfileAddresses = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      resetForm();
      setSaveMessage('Adresse enregistrée avec succès');
    } catch (err) {
  ...
  return (
    <div className="profile-addresses">
      <header>
        <h1>Mes adresses</h1>
        <p>Enregistrez vos adresses d’expéditeur, de destinataire et de facturation pour préparer vos envois plus rapidement.</p>
        <button type="button" className="dx-button dx-button--primary" onClick={() => document.getElementById('address-form-section')?.scrollIntoView({ behavior: 'smooth', block: 'start' })}>Ajouter une adresse</button>
      </header>

      {error ? <ErrorState title="Impossible de charger vos adresses" message={error} onRetry={loadAddresses} /> : null}
      {loading ? <LoadingState message="Chargement des adresses…" /> : null}
      {saveMessage ? <SuccessState title={saveMessage} /> : null}
        ...
        {(['sender', 'recipient', 'billing']).map((type) => (
          <article key={type} className="address-column">
            ...
            {groupedAddresses[type].length === 0 ? (
              <EmptyState title="Aucune adresse enregistrée" helper="Ajoutez une adresse pour accélérer vos prochaines demandes." />
            ) : (
      ...
      <section className="address-form-wrapper" id="address-form-section">
        ...
        <form onSubmit={handleSubmit} className="address-form">
          ...
          <div className="geo-section">
            <button type="button" className="toggle-advanced" onClick={() => setShowAdvancedGps((prev) => !prev)}>{showAdvancedGps ? 'Masquer les champs GPS avancés' : 'Afficher les champs GPS avancés'}</button>
            {showAdvancedGps ? <>
