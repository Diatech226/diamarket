...
export const ShipmentCard = ({ shipment }) => <article className="dx-card"><div className="dx-quote-card__head"><strong>{shipment.trackingCode || shipment._id?.slice(-8) || '—'}</strong><StatusBadge status={shipment.status} /></div><p className="dx-card__subtitle">{shipment.origin || 'Origine non renseignée'} → {shipment.destination || 'Destination non renseignée'}</p><p className="dx-card__subtitle">{shipment.transportType || shipment.serviceType || 'Transport non précisé'}</p><div className="dx-actions"><Link href="/track-shipment" className="dx-button dx-button--outline dx-button--sm">Suivre</Link></div></article>;
